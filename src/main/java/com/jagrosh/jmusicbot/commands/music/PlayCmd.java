...
public class PlayCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event) 
    {
        ...
        if(args.contains("https://www.youtube.com/shorts/")){
            args = args.replace("https://www.youtube.com/shorts/", "https://www.youtube.com/watch?v=");
        }
        String finalArgs = args;
        event.reply(loadingEmoji+" Loading... `["+args+"]`", m -> bot.getPlayerManager().loadItemOrdered(event.getGuild(), finalArgs, new ResultHandler(m,event,false)));
    }
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist)
        {
            ...
            AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
            int pos = handler.addTrack(new QueuedTrack(track, new RequestMetadata(track, event)))+1;
        ...
        private int loadPlaylist(AudioPlaylist playlist, AudioTrack exclude)
        {
            ...
            playlist.getTracks().stream().forEach((track) -> {
                ...
                {
                    AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
                    handler.addTrack(new QueuedTrack(track, new RequestMetadata(track, event)));
                    count[0]++;
                }
            });
            return count[0];
    ...
    public class PlaylistCmd extends MusicCommand
    {
        ...
        public void doCommand(CommandEvent event) 
        {
            ...
            {
                AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
                playlist.loadTracks(bot.getPlayerManager(), (at)->handler.addTrack(new QueuedTrack(at, new RequestMetadata(at, event))), () -> {
